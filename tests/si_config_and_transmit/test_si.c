...
int main(void) {
    ...
    //ph_status = check_pkt_handler_status(&si4463_device);
    if (ph_status != 0) {
        DEBUG_ERROR("Packet Handler Status Error: %d\r\n", ph_status);
        while (1) {
            HAL_GPIO_TogglePin(User_LED_GPIO_Port, User_LED_Pin);  // Blink LED on error
            HAL_Delay(500);
        }
    } else {
        DEBUG_INFO("Packet Handler Status OK\r\n");
    }

...
/**
 * @brief Initializes a telemetryInfoFrame structure with default values.
 *
 * This function dynamically allocates memory for a telemetryInfoFrame structure and 
 * initializes its fields to default values. The `typeIdentifier` is set to 'T', 
 * arrays for analog and digital values are zeroed out, and pointers for comments 
 * and telemetry data are set to NULL with their respective sizes set to zero.
 *
 * @return A pointer to the initialized telemetryInfoFrame structure if memory allocation 
 *         is successful, or NULL if memory allocation fails.
 * 
 * @note The caller is responsible for freeing the memory allocated for the telemetryInfoFrame.
 */
...
/**
 * @brief Updates the telemetryInfoFrame with new data.
 *
 * This function updates the fields of the provided telemetryInfoFrame structure with new 
 * values for the sequence number, analog values, digital values, and comment. The provided 
 * analog and digital values are copied into the respective fields of the structure using 
 * `memcpy`. The comment string is assigned to the `comment` field, and the `commentLength` 
 * is calculated as the length of the comment string.
 *
 * @param tFrame A pointer to the telemetryInfoFrame structure that will be updated.
 * @param analogValue1 A pointer to the new analog value 1 to be stored in the frame.
 * @param analogValue2 A pointer to the new analog value 2 to be stored in the frame.
 * @param analogValue3 A pointer to the new analog value 3 to be stored in the frame.
 * @param analogValue4 A pointer to the new analog value 4 to be stored in the frame.
 * @param analogValue5 A pointer to the new analog value 5 to be stored in the frame.
 * @param digitalValue A pointer to the new digital values to be stored in the frame.
 * @param comment A pointer to the new comment string to be stored in the frame.
 * 
 * @note The `sequenceNumber` field is updated with a predefined constant (`SEQUENCE_NUMBER`).
 *       The telemetry data (`tData` and `tDataSize`) are left unmodified.
 *       The caller should ensure that the `comment` string is valid and null-terminated.
 */
...
/**
 * @brief Concatenates the telemetry data into a single buffer.
 *
 * This function concatenates the telemetry data fields of the provided telemetryInfoFrame 
 * structure into a single buffer. The buffer is allocated dynamically to accommodate the 
 * concatenated data, including the comment string. The `tDataSize` field of the structure 
 * is updated with the size of the concatenated data. The concatenated data buffer is assigned 
 * to the `tData` field of the structure.
 *
 * @param tFrame A pointer to the telemetryInfoFrame structure containing the telemetry data.
 * 
 * @note The caller is responsible for freeing the memory allocated for the concatenated telemetry data.
 */
...
/**
 * @brief Initializes a telemetryParamFrame structure with default values.
 *
 * This function dynamically allocates memory for a telemetryParamFrame structure and 
 * initializes its fields to default values. The `typeIdentifier` is set to 'PARM.', 
 * and arrays for analog and digital values are zeroed out.
 *
 * @return A pointer to the initialized telemetryParamFrame structure if memory allocation 
 *         is successful, or NULL if memory allocation fails.
 * 
 * @note The caller is responsible for freeing the memory allocated for the telemetryParamFrame.
 */
...
/**
 * @brief Updates the telemetryParamFrame with new data.
 *
 * This function updates the fields of the provided telemetryParamFrame structure with new 
 * values for the analog and digital parameters. The provided analog and digital values are 
 * copied into the respective fields of the structure using `strcpy`. The `paramFrameSize` 
 * field of the structure is updated with the total size of the concatenated parameters.
 *
 * @param tParamFrame A pointer to the telemetryParamFrame structure that will be updated.
 * @param A1 A pointer to the new analog value 1 to be stored in the frame.
 * @param A2 A pointer to the new analog value 2 to be stored in the frame.
 * @param A3 A pointer to the new analog value 3 to be stored in the frame.
 * @param A4 A pointer to the new analog value 4 to be stored in the frame.
 * @param A5 A pointer to the new analog value 5 to be stored in the frame.
 * @param B1 A pointer to the new digital value 1 to be stored in the frame.
 * @param B2 A pointer to the new digital value 2 to be stored in the frame.
 * @param B3 A pointer to the new digital value 3 to be stored in the frame.
 * @param B4 A pointer to the new digital value 4 to be stored in the frame.
 * @param B5 A pointer to the new digital value 5 to be stored in the frame.
 * @param B6 A pointer to the new digital value 6 to be stored in the frame.
 * @param B7 A pointer to the new digital value 7 to be stored in the frame.
 * @param B8 A pointer to the new digital value 8 to be stored in the frame.
 * 
...
/**
 * @brief Concatenates the parameter data into a single buffer.
 *
 * This function concatenates the parameter data fields of the provided telemetryParamFrame 
 * structure into a single buffer. The buffer is allocated dynamically to accommodate the 
 * concatenated data. The concatenated data buffer is assigned to the `paramFrame` field of 
 * the structure.
 *
 * @param tParamFrame A pointer to the telemetryParamFrame structure containing the parameter data.
 * 
...
/** 
 * @brief Initializes a telemetryUnitFrame structure with default values.
 *
 * This function dynamically allocates memory for a telemetryUnitFrame structure and 
 * initializes its fields to default values. The `typeIdentifier` is set to 'UNIT.', 
 * and arrays for analog and digital values are zeroed out.
 *
 * @return A pointer to the initialized telemetryUnitFrame structure if memory allocation 
 *         is successful, or NULL if memory allocation fails.
 * 
 * @note The caller is responsible for freeing the memory allocated for the telemetryUnitFrame.
 */
...
/**
 * @brief Updates the telemetryUnitFrame with new data.
 *
 * This function updates the fields of the provided telemetryUnitFrame structure with new 
 * values for the analog and digital unit data. The provided analog and digital values are 
 * copied into the respective fields of the structure using `strcpy`.
 *
 * @param tUnitFrame A pointer to the telemetryUnitFrame structure that will be updated.
 * @param A1 A pointer to the new analog value 1 to be stored in the frame.
 * @param A2 A pointer to the new analog value 2 to be stored in the frame.
 * @param A3 A pointer to the new analog value 3 to be stored in the frame.
 * @param A4 A pointer to the new analog value 4 to be stored in the frame.
...
/**
 * @brief Concatenates the unit data into a single buffer.
 *
 * This function concatenates the unit data fields of the provided telemetryUnitFrame 
 * structure into a single buffer. The buffer is allocated dynamically to accommodate the 
 * concatenated data. The concatenated data buffer is assigned to the `unitFrame` field of 
 * the structure.
 *
 * @param tUnitFrame A pointer to the telemetryUnitFrame structure containing the unit data.
 * 
